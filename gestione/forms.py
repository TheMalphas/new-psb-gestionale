...
class AddTrasferteFormGestione(forms.ModelForm):
    class Meta:
        model = AddTrasferte
        exclude = ["id_trasferte","id_dip","id_ced","rel_giorno","stato","ore","timestamp","note"]
...
class ContrattiFormInsert(forms.ModelForm):
    dipendente = forms.ModelChoiceField(queryset=AnaDipendenti.objects.filter(stato='Attivo').filter(id_contratto__isnull=True),widget=forms.Select(attrs={"class":"form-control","id":"dipendente","name":"dipendente"}))
...
class AnaDipendentiCarloForm(forms.ModelForm):
    class Meta:
        model= AnaDipendenti
        exclude = ['id_dip','user','id_contratto','id_societa','data_creazione','created_at']
