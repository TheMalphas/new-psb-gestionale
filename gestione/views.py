...
class DashboardContrattiView(LoginRequiredMixin, PermissionRequiredMixin,View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        contrInScadSett = Contratti.objects.filter(datafine__range=(self.today,self.settimana)).count()
        contrInScadQuind = Contratti.objects.filter(datafine__range=(self.today,self.quindicina)).count()
        contrInScadMese = Contratti.objects.filter(datafine__range=(self.today,self.mese)).order_by("datafine")
        contrScaduti = Contratti.objects.filter(datafine__lte=self.today).order_by("datafine")
...
class ContrattiCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        dipendente = form.cleaned_data['dipendente']
        dipendente_id = dipendente.pk
        temp = form.save()
        print(temp)
        temp.id_dip = AnaDipendenti.objects.get(id_dip=dipendente_id)
        temp.save()
        return super().form_valid(form)  
...
def gestioneTrasfertePersonale(request):
    ...
    today = datetime.now().date()
    monthDays =  (datetime.today().date() + timedelta(30))
    ...
    try:
        ...
        elif request.method == "POST" and request.POST.get("rifiutate") == "rifiutate":
            queryset = AddTrasferte.objects.filter(stato=2).order_by("-timestamp_creazione")
            
            return render(request, template_name, {'richieste':queryset})
           
        elif request.method == "POST" and request.POST.get("accettate") == "accettate":
            queryset = AddTrasferte.objects.filter(stato=1).order_by("-timestamp_creazione")
            
            return render(request, template_name, {'richieste':queryset})
        
        elif request.method == "POST" and request.POST.get("dacontrollare") == "dacontrollare":
...
class RichiesteTrasferteUpdate(LoginRequiredMixin,UpdateView):
    model= AddTrasferte
    template_name = "gestione/conferma-richieste-trasferte.html"
    context_object_name = "richiesta"
    form_class = AddTrasferteFormGestione
    success_url= reverse_lazy('gestione:richieste-trasferte')

    def get_object(self):
        id_trasferte = self.kwargs.get("id_trasferte")
        AddTrasferte.objects.filter(id_trasferte=id_trasferte).update(stato=1)
        return get_object_or_404(AddTrasferte, id_trasferte=id_trasferte)

class RichiesteTrasferteRifiuta(UpdateView):
    model = AddTrasferte
    fields = [] # empty fields list means no form will be displayed
    context_object_name = "richiesta"
    template_name = "gestione/rifiuta-richieste-trasferte.html"
    success_url= reverse_lazy('gestione:richieste-trasferte')
