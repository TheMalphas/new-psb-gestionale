    ...
    def __str__(self):
        ...
        if self.stato == 0:
            if self.id_permessi_richieste:
                return f"Richiesta del {str(day[0])} per {Permessi.objects.get(tipopermesso=self.id_permessi_richieste)}: rifiutata."
            else: return f"Richiesta di permesso orario del {str(day[0])}: rifiutata."
        elif self.stato == 1:
            if self.id_permessi_richieste:
                return f" Richiesta del {str(day[0])} per {Permessi.objects.get(tipopermesso=self.id_permessi_richieste)}. Confermata."
            else: return f"Richiesta di permesso orario del {str(day[0])}. Confermata."
        else:
            if self.id_permessi_richieste:
                return f"Richiesta del {str(day[0])} per {Permessi.objects.get(tipopermesso=self.id_permessi_richieste)}: in attesa di revisione."
            else: return f"Richiesta di permesso orario del {str(day[0])}: in attesa di revisione."
