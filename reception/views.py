...
from .models import Richieste, AuthUser, AnaDipendenti, BancaOrari, CapoArea, Permessi, RichiesteAccettate, Ingressidip, Ritardi_Anticipi, Area, Societa, Sede
...
class ListaGiorniPresenzeDip(LoginRequiredMixin,PermissionRequiredMixin,ListView):
    ...
    #     anno = datetime.now().date().year
    #     queryset = Ingressidip.objects.filter(id_dip_in=).exclude(area__in=self.esclusioni).order_by("cognome")
...
def listaPresenze(request):
    ...
    today = datetime.now().date()
    excludeIdList = [9,23,24,31,32]
    presenze = Ingressidip.objects.filter(giorno=today,id_dip_ing__stato='Attivo').exclude(id_dip_ing__area__in=excludeIdList).select_related("id_dip_ing").order_by("nominativo")
    context= {"presenze":presenze}
    totale_entrate = presenze.filter(giorno=today,checked_in=True).count()
    totale_uscite = presenze.filter(giorno=today,checked_out=True).count()
    totale_assenze = presenze.filter(giorno=today,checked_in=False,in_permesso=False).count()
    totale_permessi = presenze.filter(giorno=today,checked_in=False,in_permesso=True).count()
    totale_ferie = presenze.filter(giorno=today,id_permesso=6).count()
    context["totale_entrate"] = totale_entrate    
    context["totale_uscite"] = totale_uscite
    context["totale_assenze"] = totale_assenze
    context["totale_permessi"] = totale_permessi
    context["totale_ferie"] = totale_ferie
    context["data"] = today
    template_name = "reception/presenze.html"

    if request.method == "POST" and request.POST.get("giorno") == None or (request.POST.get("giorno") == "") :
        ...
        totale_uscite = presenze.filter(giorno=today,checked_in=True,checked_out=True,timestamp_scan_uscita__isnull=False,).count()
        ...
        presenze = Ingressidip.objects.filter(giorno=date,id_dip_ing__stato='Attivo').exclude(id_dip_ing__area__in=excludeIdList).select_related("id_dip_ing").order_by("nominativo")
        context= {"presenze":presenze}
        totale_entrate = presenze.filter(giorno=date).count()
        totale_uscite = presenze.filter(giorno=date).count()
        totale_assenze = presenze.filter(giorno=date,in_permesso=False).count()
        ...
        context["totale_ferie"] = totale_ferie
        context["data"] = date
        return render(request, template_name, {"presenze":presenze,"totale_entrate":totale_entrate,"totale_uscite":totale_uscite,"totale_assenze":totale_assenze,"totale_permessi":totale_permessi,"totale_ferie":totale_ferie})
    elif request.method == "POST" and (request.POST.get("giorno") != None):
        data_object = request.POST.get("date")
        date = datetime.strptime(data_object, "%Y-%m-%d").date()
        presenze = Ingressidip.objects.filter(giorno=date,id_dip_ing__stato='Attivo').exclude(id_dip_ing__area__in=excludeIdList).select_related("id_dip_ing").order_by("nominativo")
        context= {"presenze":presenze}
        totale_entrate = presenze.filter(giorno=date).count()
...
class EntrateQR(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def registraPulizie(self, user, id_dip,request, context, *args, **kwargs):
        today = datetime.now().date()
        timestamp = datetime.now()
        # Checks if it is a user id
        try:
            id_dip = int(id_dip)

        except:
            # If check fails, stop query process and displays failure result
            errore= "Errore"
            ...
            if type(id_dip) != int:
                # If not of int type, displays failure result
                errore= "Errore"
                context["errore"] = errore
                return render(self.request,self.template_name,context)
            
            else:
                dipendenti = AnaDipendenti.objects.filter(id_dip=id_dip).exists()
    ...
    def registerQuery(self, user, id_dip,request, context, *args, **kwargs):
        today = datetime.now().date()
        timestamp = datetime.now()
        ...
        if timestamp.hour < 12:
            ...
            try:
                ...
                if type(id_dip) != int:
                    ...
                    dipendenti = AnaDipendenti.objects.filter(id_dip=id_dip).exists()
                    entrato = Ingressidip.objects.filter(id_dip_ing = id_dip, giorno=today,checked_in=False).exists()

                    if dipendenti and entrato:
            
                        # Correct procedure, registers entrance
                        dip = AnaDipendenti.objects.get(id_dip=id_dip)
                        cancelliere = AuthUser.objects.get(id=self.request.user.pk)
                        instance = Ingressidip.objects.filter(id_dip_ing = id_dip, giorno=today).update(registrato_da_user=cancelliere,entrata=timestamp,checked_in=True,timestamp_scan_entrata=timezone.now() + timezone.timedelta(hours=1))
                        dipendente = "dipendente"
                        ...
                        context["entrato"] = entrato
                        return render(self.request,self.template_name,context) 
                    elif dipendenti and not(entrato):               
                        # Warns of already registered user
                        registratoEntrata = "registrato-entrata"
                        context["registratoEntrata"] = registratoEntrata
                        return render(self.request, self.template_name, context)
            ...
            try:
                ...
                if type(id_dip) != int:
                    ...
                    dipendenti = AnaDipendenti.objects.filter(id_dip=id_dip).exists()
                    entrato = Ingressidip.objects.filter(id_dip_ing = id_dip, giorno=today,checked_in=True).exists()
                    uscito = Ingressidip.objects.filter(id_dip_ing = id_dip, giorno=today,checked_out=False).exists()

                    if dipendenti and not(entrato):
                        # Warns if user is trying to register an exit but didn't register an entrance
                        nonEntrato = "non-entrato"
                        context["nonEntrato"] = nonEntrato
                        return render(self.request, self.template_name, context)
                    elif dipendenti and uscito:
                        # Correct procedure, registers entrance
                        dip = AnaDipendenti.objects.get(id_dip=id_dip)
                        cancelliere = AuthUser.objects.get(id=self.request.user.pk)
                        instance = Ingressidip.objects.filter(id_dip_ing = id_dip, giorno=today).update(registrato_da_user=cancelliere,uscita=timestamp,checked_out=True,timestamp_scan_uscita=timezone.now() + timezone.timedelta(hours=1))
                        dipendente = "dipendente"
                        uscito = "uscito"
                        context["dipendente"] = dipendente
                        context["dip"] = dip
                        context["uscito"] = uscito
                        return render(self.request,self.template_name,context)
                    elif dipendenti and not(uscito):               
                        # Warns of already registered user
                        registratoUscita = "registrato-uscita"
                        context["registratoUscita"] = registratoUscita
                        return render(self.request, self.template_name, context)
    ...
    def post(self, request, id_dip_ing=None, *args, **kwargs):
        ...
        try:
            id_dip_ing = request.POST["id_dip_ing"]
            dipendente = AnaDipendenti.objects.filter(id_dip=id_dip_ing).values_list('area', flat=True)
            dipendentesede = AnaDipendenti.objects.filter(id_dip=id_dip_ing).values_list('sede',flat=True)
            dipper = AnaDipendenti.objects.get(id_dip=id_dip_ing)
            areas = Area.objects.get(id_area=dipendente[0])
            sedes = Sede.objects.get(id_sede=dipendentesede[0])
            if request.method == "POST" and request.POST.get("id_dip_ing"):
                current_user = request.user
                user = current_user.id
                self.registraPulizie(user,id_dip_ing,request, context)
                return render(request, self.template_name, context)
            # elif request.method == "POST" and request.POST.get("id_dip_ing") != None or dipper.id_dip in listadoppiturni:
            #     current_user = request.user
            #     user = current_user.id
            #     self.registraPulizie(user,id_dip_ing,request, context,areas)
            #     return render(request, self.template_name, context)
            # elif request.method == "POST" and request.POST.get("id_dip_ing") != None and areas.id_area != 21:
            #     current_user = request.user
            #     user = current_user.id
            #     self.registerQuery(user,id_dip_ing,request, context)
            #     return render(request, self.template_name, context)
            else: 
                return render(request, self.template_name, context)
